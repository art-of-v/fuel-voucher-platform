...
| 9 | Review and prune the test-phone fixed-OTP allowlist | Each entry is a permanent, non-rotating OTP for that number — a production credential | The startup warning reports the expected count. If it reports 0, better | **DONE (removed)** — testing period ended; the whole test-phone mechanism was deleted from code, compose and docs on 2026-09-15, so no fixed OTP can exist in Production anymore |
...
| Startup logs on every deploy | `ValidateSecurityConfiguration` warnings are the only signal that DeviceAuth is off | Grep every deploy's startup log for `SECURITY:` |
