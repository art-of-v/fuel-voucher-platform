...
| Race `mark-used` to redeem twice | At audit time WP-4 was genuinely unimplemented (read-then-write) — but the outcome was **benign**: an already-`Used` voucher returns success, so the result is idempotent, no money moves, and the worst case was a redundant write. Authorization was also correct (`WorkerUserId`, then `AssignedToUserId`). **This entry has since been superseded: the guard now exists** (conditional `ExecuteUpdateAsync`, 2026-08-22), so this row records an absence of consequence that no longer has to carry the weight | Was: not a guard so much as an absence of consequence. Now: `MarkVoucherAsUsedCommandHandler`, atomic conditional `UPDATE` — **tests executed 2026-08-23, 2 passed, negative control fails against the pre-fix handler**, see [07-coverage-statement.md](07-coverage-statement.md#test-execution-once-docker-became-available--2026-08-23) |
...
|---|---|
| **A live Supabase database password is in git history** (`REMEDIATION_PRIORITIES.md` P0 F4, `FRAUD_ANALYSIS.md:22`) | **REFUTED, and re-confirmed 2026-08-22 by a stronger method.** The three cited commits do not exist here — `git cat-file -t` returns `fatal: Not a valid object name` for `7ff3dae1`, `0ff95631` and `aa36b124`. The original sweep of 2,622 text blobs found 23 distinct postgres-URI credentials, **every one a placeholder** (`postgres`, `password`, `PASSWORD` paired with username `USER` in `render.yaml`, a 4-char entropy-0.0 masked log value). The re-check went further: extracting every distinct value in the password position for the specific Supabase host, across **all 3,766 blobs in the object database**, yields exactly one string in ten occurrences — the literal `***REDACTED***`. The host and `postgres.<ref>` username *are* in five commits; those are not credentials. **Caveat stated rather than hidden:** this proves the current object database is clean, not that no live value ever reached a remote. That question is answered by the owner's rotation attestation, not by this repository. Source doc corrected — see [04-doc-claim-verification.md](04-doc-claim-verification.md) |
