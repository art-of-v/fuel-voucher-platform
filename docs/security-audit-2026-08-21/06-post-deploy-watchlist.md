...
| `FUELFLOW_ACK_UNSIGNED_OTA` in `.github/workflows/ci.yml` | The FF-03 CI gate exists but is set to *report* rather than block, via that flag. It is the difference between a Critical that is acknowledged and one that is enforced | Delete the four-line `env:` block the moment signing ships — the gate then goes hard automatically. If it is still set on **2026-11-30**, re-review the acceptance rather than letting it become permanent |
| `npm audit` / `nuget audit` once egress permits | Both were unreachable during this audit. [FF-33](03-findings-medium-low.md#ff-33--sshnet-202510-high-severity-cve--info-confirmed-fixed-2026-08-22) is now **fixed** (pinned to `2026.0.0`, `NU1903` gone), but it was found in build warnings rather than an advisory sweep — **other vulnerable dependencies may exist that nobody has looked for** | Run both in week one and re-triage. Finding nothing is only meaningful once the commands have actually run |
