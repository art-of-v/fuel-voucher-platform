...
**Status:** ⏳ open. The build emits `NU1903` for `Newtonsoft.Json` 11.0.1 pulled into
`FuelFlow.IntegrationTests`, `FuelFlow.UnitTests`, and `FuelFlow.JobsWorker.UnitTests`
(advisory GHSA-5crp-9r3c-p9vr). It arrives transitively (likely via a test/testcontainers
dependency). **Action:** add a direct pinned reference to a patched `Newtonsoft.Json`
(≥ 13.0.3) in the test projects, or migrate those call sites to `System.Text.Json`.
Note the repo has a `gitleaks` secret-scan gate but no dependency-vuln gate — worth a
`dotnet list package --vulnerable` CI step.
