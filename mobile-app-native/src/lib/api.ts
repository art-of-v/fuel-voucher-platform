...
export async function getMyVouchers(): Promise<Voucher[]> {
  const response = await apiFetch('/api/vouchers/my', { credentials: 'include' });
  if (!response.ok) {
...
export async function getMyOrders(): Promise<Order[]> {
  const response = await apiFetch('/api/sync/orders', { credentials: 'include' });
  if (!response.ok) {
...
export async function getMyPurchases(): Promise<PurchaseResponse[]> {
  const response = await apiFetch('/api/purchases/my', { credentials: 'include' });
  if (!response.ok) {
    if (response.status === 401) {
