import { create } from 'zustand';
import { persist } from 'zustand/middleware';

export type Language = 'en' | 'uk' | 'de' | 'es';

export const languages: { code: Language; name: string; flag: string }[] = [
    { code: 'en', name: 'English', flag: '🇬🇧' },
    { code: 'uk', name: 'Українська', flag: '🇺🇦' },
    { code: 'de', name: 'Deutsch', flag: '🇩🇪' },
    { code: 'es', name: 'Español', flag: '🇪🇸' },
];

const translations: Record<Language, Record<string, string>> = {
    en: {
        // App
        'app.title': 'FUEL FLOW',
        'app.admin': 'ADMIN',

        // Sidebar Navigation
        'nav.management': 'Management',
        'nav.fueltypes': 'Fuel Types',
        'nav.purchases': 'Purchases',
        'nav.users': 'Users',
        'nav.providers': 'Providers',
        'nav.vouchers': 'Import Vouchers',
        'nav.imports': 'Imports',
        'nav.contracts': 'Contracts',
        'nav.reconciliation': 'Audit',
        'nav.auditlog': 'Audit Log',
        'nav.errorlogs': 'Error Logs',
        'nav.reports': 'Reports',
        'nav.settings': 'Settings',

        // Settings
        'settings.autoRefund': 'Auto-refund',
        'settings.enableAutoRefund': 'Enable auto-refund',
        'settings.enableAutoRefundHint': 'When enabled, partially fulfilled orders are automatically refunded once the grace period has passed.',
        'settings.gracePeriod': 'Grace period (days)',
        'settings.gracePeriodHint': '7 = one week · 30 = one month · 365 = one year',
        'settings.save': 'Save settings',
        'settings.saved': 'Settings saved',
        'settings.disabledNote': 'Auto-refund is currently OFF. Orders stay partially fulfilled until you fulfil them or refund them manually.',
        'settings.qaTitle': 'QA test access',
        'settings.qaWhat': 'Lets the dedicated QA test account sign in to the production app for App Store review, using its reserved phone number and a server-side QA code. Turn it on only while QA is in progress and off immediately afterwards.',
        'settings.qaEnable': 'Enable QA test access',
        'settings.qaEnableHint': 'When on, the QA account can sign in with its QA code. When off, that sign-in stops working and any active QA session is signed out.',
        'settings.qaStatusEnabled': 'Enabled',
        'settings.qaStatusDisabled': 'Disabled',
        'settings.qaPhone': 'QA account phone',
        'settings.qaNotConfigured': 'No QA code is configured on the server, so QA sign-in cannot work even when enabled. Set the QA code secret in the deployment before enabling.',
        'settings.qaDisabledNote': 'QA test access is OFF. The QA phone and code do not grant access.',
        'settings.qaLastChanged': 'Last changed',
        'settings.qaSaved': 'QA test access updated',

        // Imports
        'imports.title': 'Import Vouchers',
        'imports.back': 'Back to imports',
        'imports.activateAll': 'Activate all',
        'imports.deactivateAll': 'Deactivate all',
        'imports.number': 'Number',
        'imports.verification': 'Verification',
        'imports.noVouchers': 'No vouchers in this import',
        'imports.file': 'File',
        'imports.pages': 'Pages',
        'imports.vouchers': 'Vouchers',
        'imports.imported': 'Imported',
        'imports.duplicates': 'Duplicates',
        'imports.errors': 'Errors',
        'imports.warnings': 'Warnings',
        'imports.created': 'Created',
        'imports.none': 'No imports yet',
        'status.deactivated': 'Deactivated',
        'contracts.availableTitle': 'Available contracts',
        'contracts.name': 'Name',
        'contracts.version': 'Version',
        'contracts.createdAt': 'Created',
        'contracts.noneAvailable': 'No additional contracts found',
        'contracts.signedTitle': 'Signed contracts',
        'contracts.user': 'User',
        'contracts.company': 'Company',
        'contracts.contract': 'Contract',
        'contracts.signedAt': 'Signed date',
        'contracts.signature': 'Signature',
        'contracts.view': 'View',
        'contracts.noneSigned': 'No signed contracts yet',

        // Fuel Prices
        'fuelprices.packagePrices': 'Package Prices (₴/L)',
        'fuelprices.editHint': 'Click any price to edit · Tab / ↑↓ to navigate · Enter to confirm',

        // User Profile
        'user.name': 'Admin System',
        'user.role': 'Super User',

        // Header
        'header.dashboard': 'Dashboard',
        'header.docs': 'Documentation',
        'header.support': 'Support',

        // Common
        'common.loading': 'Loading...',
        'common.save': 'Save',
        'common.cancel': 'Cancel',
        'common.delete': 'Delete',
        'common.deleteAll': 'Delete All',
        'common.edit': 'Edit',
        'common.create': 'Create',
        'common.search': 'Search...',
        'common.actions': 'Actions',
        'common.status': 'Status',
        'common.date': 'Date',
        'common.print': 'Print',
        'common.close': 'Close',
        'common.add': 'Add',
        'common.history': 'History',
        'common.noData': 'No data',
        'common.saved': 'Saved',
        'common.deleted': 'Deleted',
        'common.created': 'Created',
        'common.confirm': 'Confirm',
        'table.nominals': 'Denominations',
        'price.supplier': 'Supplier',
        'price.margin': 'Margin',
        'price.final': 'Final',
        'price.unit': 'UAH/L',
        'price.supplierPlaceholder': 'e.g. 50.00',
        'providers.addProvider': 'Add Provider',
        'providers.namePlaceholder': 'Provider Name (e.g. OKKO)',
        'providers.logoText': 'Logo Text',
        'providers.logoTextPlaceholder': 'Logo Text',
        'providers.color': 'Color',
        'providers.editProvider': 'Edit Provider',
        'providers.noProviders': 'No providers found',
        'providers.priority': 'Priority',
        'providers.priorityHint': 'Display order in the mobile app. Lower = higher. 999 = end of the list.',
        'providers.discount': 'Discount',
        'providers.discountHint': 'Marketing discount (UAH/L) shown against the pump price on the customer card. Does not change the chargeable price.',
        'providers.deleteConfirm': 'Delete {0}?',
        'providers.nominalsCount': '{0} denominations',
        'history.providerCreated': 'Created provider {0}',
        'history.providerDeleted': 'Deleted provider {0}',
        'history.providerUpdated': 'Updated provider {0}',
        'history.nominalsChanged': '{0}: denominations changed [{1}]',
        'history.fuelAdded': '{0} / {1}: added at {2} UAH/L',
        'history.fuelRemoved': '{0} / {1}: removed',
        'history.fuelUpdated': '{0} / {1}: updated',
        'history.changeName': 'name {0} → {1}',
        'history.changeSupplier': 'supplier {0} → {1}',
        'history.changeMargin': 'margin {0} → {1}',
        'history.changeFinal': 'final {0} → {1}',
        'history.changeLogo': 'logo {0} → {1}',
        'history.changeColor': 'color {0} → {1}',
        'price.marginPlaceholder': 'e.g. 2.00',
        'price.finalPlaceholder': 'Final (auto)',
        'price.nominalsAuto': 'auto',
        'price.title': 'Prices',

        // Reports
        'report.allUsers': 'All Users',
        'report.title': 'User Reports',
        'report.from': 'From',
        'report.to': 'To',
        'report.yesterday': 'Yesterday',
        'report.30days': '30 Days',
        'report.week': 'Week',
        'report.month': 'Month',
        'report.generate': 'Generate Report',
        'report.reconciliationAct': 'Reconciliation Act',
        'report.loading': 'Loading...',
        'report.noData': 'Select parameters and click "Generate Report"',
        'report.totalSpent': 'Profit',
        'report.orders': 'orders',
        'report.purchased': 'Purchased',
        'report.litersPurchased': '~{0} liters',
        'report.used': 'Used',
        'report.litersUsed': 'LITERS USED',
        'report.monthlyBreakdown': 'Monthly Breakdown',
        'report.amount': 'Amount',
        'report.payments': 'Payments',
        'report.paymentId': 'ID',
        'report.provider': 'Provider',
        'report.fuel': 'Fuel',
        'report.amountHeader': 'Amount',
        'report.liters': 'Liters',
        'report.quantity': 'Qty',
        'report.bank': 'Bank',
        'report.status': 'Status',
        'report.date': 'Date',
        'report.noPayments': 'No payments',
        'report.monobankInvoices': 'Monobank invoices: {0}',
        'report.redemptions': 'Redemptions',
        'report.redemptionId': 'ID',
        'report.fuelName': 'Fuel',
        'report.redeemedAt': 'Redeemed At',
        'report.noRedemptions': 'No redemptions',
        'report.fulfilledOrders': 'fulfilled orders',
        'report.profitSummary': 'Profit Summary',
        'report.period': 'Period',
        'report.orders_short': 'Orders',
        'report.profit': 'Profit',
        'report.avgOrder': 'Avg Order',

        // Forms
        'forms.selectStation': 'Select Station',
        'forms.selectFuel': 'Select Fuel',
        'forms.stationIdPlaceholder': 'Station ID (e.g. okko)',
        'forms.stationNamePlaceholder': 'Station Name (e.g. OKKO)',
        'forms.logoTextPlaceholder': 'Logo Text',
        'forms.fuelIdPlaceholder': 'Fuel ID (e.g. a95)',
        'forms.fuelNamePlaceholder': 'e.g. A-95',
        'forms.basePricePlaceholder': 'Base Price',
        'forms.discountPricePlaceholder': 'Discount Price',
        'forms.qrUrlPlaceholder': 'QR Code URL or Image URL',
        'forms.packageIdPlaceholder': 'Package ID (e.g. okko_a95_10)',
        'forms.priceUahPlaceholder': 'Price (UAH)',
        'forms.originalPriceUahPlaceholder': 'Original Price (UAH)',

        // Dashboard
        'dashboard.availableQrs': 'Available QRs',
        'dashboard.soldQrs': 'Sold QRs',

        // Tables
        'table.id': 'ID',
        'table.name': 'Name',
        'table.logo': 'Logo',
        'table.color': 'Color',
        'table.station': 'Station',
        'table.fuel': 'Fuel',
        'table.liters': 'Liters',
        'table.price': 'Price',
        'table.number': '#',
        'table.description': 'Description',
        'table.quantity': 'Qty',
        'table.date': 'Date',
        'table.amount': 'Amount, UAH',
        'table.debit': 'Debit',
        'table.credit': 'Credit',
        'table.uah': 'UAH',
        'table.original': 'Original',
        'table.qrPreview': 'QR Preview',
        'table.basePrice': 'Base Price',
        'table.discountPrice': 'Discount Price',
        'table.phone': 'Phone',
        'table.email': 'Email',
        'table.birthdate': 'Birthdate',
        'table.bonusBalance': 'Bonus Balance',
        'table.referralCode': 'Referral Code',
        'table.referredBy': 'Referred By',
        'table.role': 'Role',
        'table.status': 'Status',
        'users.active': 'Active',
        'users.inactive': 'Inactive',
        'users.deleted': 'Deleted',
        'users.createdAt': 'Created Date',
        'users.activate': 'Activate',
        'users.deactivate': 'Deactivate',
        'users.activated': 'User activated',
        'users.deactivated': 'User deactivated',
        'users.delete': 'Delete',
        'users.deleteConfirm': 'Delete this user? This permanently blocks their account.',
        'users.deleteSuccess': 'User deleted',
        'users.deleteFailed': 'Failed to delete user',
        'users.toggleFailed': 'Failed to change user status',
        'users.banned_status': 'Banned',
        'users.ban': 'Ban',
        'users.unban': 'Unban',
        'users.banConfirm': 'Ban this user? All their active sessions end immediately and they must sign in again after being unbanned.',
        'users.banned': 'User banned',
        'users.unbanned': 'User unbanned',
        'users.banFailed': 'Failed to change ban status',
        'users.roleChanged': 'Role updated',
        'users.roleChangeFailed': 'Failed to change role',
        'users.editEmail': 'Edit email',
        'users.emailPrompt': 'New email (leave blank to clear). A confirmation link is sent to the new address before it takes effect.',
        'users.emailChangeSent': 'Confirmation link sent to the new address',
        'users.emailCleared': 'Email cleared',
        'users.emailChangeFailed': 'Failed to change email',

        // Purchases
        'table.vouchers': 'Vouchers',
        'purchases.refund': 'Refund',
        'purchases.refundTitle': 'Confirm refund',
        'purchases.refundConfirm': 'Refund this order? Unfulfilled value of {0} UAH will be returned to the customer.',
        'purchases.refundNothingToRefund': 'No unfulfilled value remains on this order.',
        'purchases.refundStatus.Processing': 'Refund pending',
        'purchases.refundStatus.Completed': 'Refund confirmed',
        'purchases.refundStatus.Failed': 'Refund failed',
        'purchases.refundSuccess': 'Refund initiated: {0} UAH',
        'purchases.refundFailed': 'Refund failed',

        // Vouchers / Import
        'import.title': 'Import Vouchers',
        'import.description': 'Upload PDF or Image files to import vouchers into the system.',
        'import.dragDrop': 'Drag & drop voucher images, PDFs, or ZIP here',
        'import.clickSelect': 'Click to select',
        'import.uploading': 'Uploading...',
        'import.processing': 'Processing...',
        'import.largeFileNote': 'Large files may take a few minutes to process.',
        'import.completed': 'Completed',
        'import.failed': 'Failed',
        'import.totalFiles': 'Total Files',
        'import.processed': 'Processed',
        'import.successful': 'Successful',
        'import.failedCount': 'Failed',
        'import.start': 'Start Import',
        'import.filesSelected': 'Selected {0} file(s):',
        'import.close': 'Close',
        'import.model': 'Model',
        'import.errorOccurred': 'An error occurred during import.',
        'import.duplicates': 'Duplicates',
        'import.noVouchers': 'No vouchers imported yet.',
        'import.scanTitle': 'Scan Voucher',

        // Voucher Table
        'vouchers.id': 'ID',
        'vouchers.image': 'Image',
        'vouchers.volume': 'Volume',
        'vouchers.fuelType': 'Fuel Type',
        'vouchers.provider': 'Provider',
        'vouchers.status': 'Status',
        'vouchers.importedAt': 'Imported At',
        'vouchers.expires': 'Expires',
        'vouchers.externalId': 'External ID',

        // Voucher Status
        'status.imported': 'Imported',
        'status.available': 'Available',
        'status.assigned': 'Assigned',
        'status.used': 'Used',
        'status.sold': 'Sold',
        'status.expired': 'Expired',
        'status.reserved': 'Reserved',
        'status.verificationFailed': 'Verification Failed',
        'status.active': 'Active',
        'status.delivered': 'Delivered',
        'status.pending': 'Pending',
        'status.pending_qr': 'Pending QR',
        'status.completed': 'Completed',
        'status.failed': 'Failed',
        'status.started': 'Started',
        'status.verifiedWithWarnings': 'Verified With Warnings',

        // Vouchers
        'vouchers.total': 'Total',
        'vouchers.filtered': 'Filtered',
        'vouchers.selected': 'Selected',
        'vouchers.allFuelTypes': 'All Fuel Types',
        'vouchers.allStatuses': 'All Statuses',
        'vouchers.allProviders': 'All Providers',
        'vouchers.all': 'All',
        'vouchers.clearFilters': 'Clear Filters',
        'vouchers.delete': 'Delete',
        'vouchers.deleteConfirmTitle': 'Confirm Deletion',
        'vouchers.deleteConfirm': 'Are you sure you want to delete {0} vouchers? This action cannot be undone.',
        'vouchers.deleteAllTitle': 'Delete All Vouchers',
        'vouchers.deleteAllConfirm': 'Are you sure you want to delete all {0} vouchers? This action cannot be undone.',
        'vouchers.deleteAllAction': 'Delete All',
        'vouchers.cancel': 'Cancel',

        // Order Statuses
        'order.status.pendingPayment': 'Pending Payment',
        'order.status.paid': 'Paid',
        'order.status.pendingFulfillment': 'Pending Fulfillment',
        'order.status.fulfilled': 'Fulfilled',
        'order.status.partiallyFulfilled': 'Partially Fulfilled',
        'order.status.partiallyRefunded': 'Partially Refunded',
        'order.status.pending': 'Pending',
        'order.status.cancelled': 'Cancelled',
        'order.status.refunded': 'Refunded',

        // Monobank
        'monobank.status.success': 'Success',
        'monobank.status.pending': 'Pending',

        // Reconciliation
        'reconciliation.title': 'Audit',
        'reconciliation.summary': 'Summary',
        'reconciliation.revenue': 'Revenue',
        'reconciliation.fulfilled': 'Fulfilled',
        'reconciliation.pending': 'Pending',
        'reconciliation.unprocessed': 'Unprocessed Events',
        'reconciliation.importErrors': 'Import Errors (7d)',
        'reconciliation.pendingOrders': 'Pending Orders',
        'reconciliation.voucherInventory': 'Voucher Inventory',
        'reconciliation.monthlyRevenue': 'Monthly Revenue',
        'reconciliation.recentImports': 'Recent Imports',
        'reconciliation.noPending': 'No pending orders',
        'reconciliation.noImports': 'No recent imports',
        'reconciliation.modalTitle': 'Reconciliation Act',
        'reconciliation.documentTitle': 'RECONCILIATION ACT',
        'reconciliation.periodLabel': 'Period:',
        'reconciliation.periodFrom': 'from the beginning',
        'reconciliation.periodTo': 'to today',
        'reconciliation.userLabel': 'User:',
        'reconciliation.noOperations': 'No operations for the selected period',
        'reconciliation.paymentDesc': 'Voucher purchase: {0} {1}L x{2}',
        'reconciliation.redemptionDesc': 'Voucher redemption: {0} {1}L',
        'reconciliation.total': 'Total:',
        'reconciliation.balance': 'Ending Balance:',
        'reconciliation.summaryInfo': 'Summary Information:',
        'reconciliation.totalOrders': 'Total orders: {0}',
        'reconciliation.vouchersPurchased': 'Vouchers purchased: {0} pcs ({1} L)',
        'reconciliation.vouchersUsed': 'Vouchers used: {0} pcs ({1} L)',
        'reconciliation.fuel': 'fuel',
        'reconciliation.adminSignature': 'Admin signature:',
        'reconciliation.userSignature': 'User signature:',
        'reconciliation.needAttention': 'Need Attention',
        'reconciliation.unfulfilled': 'unfulfilled',
        'reconciliation.partial': 'partial',
        'reconciliation.dataIntegrity': 'Data Integrity',
        'reconciliation.orphanVouchers': 'orphan vouchers / unprocessed events',
        'reconciliation.providersDeficit': 'Providers with Deficit',
        'reconciliation.importErrors7d': 'import errors (7d)',
        'reconciliation.issuesFound': 'Issues Found ({0})',
        'reconciliation.critical': 'CRITICAL',
        'reconciliation.warning': 'WARNING',
        'reconciliation.threeWayMatch': 'Three-Way Match · Order — Payment — Fulfillment',
        'reconciliation.threeOrder': 'Order',
        'reconciliation.threePayment': 'Payment',
        'reconciliation.threeFulfillment': 'Fulfillment',
        'reconciliation.threeVouchExp': 'Vouch. Exp',
        'reconciliation.threeVouchDel': 'Vouch. Del',
        'reconciliation.threeMatch': 'Match',
        'reconciliation.threeAge': 'Age',
        'reconciliation.matchOk': 'OK',
        'reconciliation.matchPartial': 'PARTIAL',
        'reconciliation.matchUnfulfilled': 'UNFULFILLED',
        'reconciliation.matchCancelled': 'CANCELLED',
        'reconciliation.matchRefunded': 'REFUNDED',
        'reconciliation.matchPartial_refunded': 'PARTIAL + REFUND',
        'reconciliation.threeRefund': 'Refund',
        'reconciliation.moneyLedger': 'Money Ledger · received — delivered — refunded',
        'reconciliation.received': 'Received',
        'reconciliation.deliveredValue': 'Delivered value',
        'reconciliation.refundedLabel': 'Refunded',
        'reconciliation.refundedOrders': 'refunded orders',
        'reconciliation.refundDesc': 'Refund of undelivered vouchers: {0}',
        'reconciliation.noOrderData': 'No order data',
        'reconciliation.voucherFunnel': 'Voucher Funnel',
        'reconciliation.loadFailed': 'Failed to load reconciliation data',

        // Audit Log
        'auditlog.title': 'Audit Log',
        'auditlog.eventType': 'Event',
        'auditlog.aggregate': 'Entity',
        'auditlog.changedBy': 'Changed By',
        'auditlog.date': 'Date',
        'auditlog.summary': 'Summary',
        'auditlog.noEvents': 'No events recorded yet.',
        'auditlog.total': 'Total',

        // Error Logs
        'errorlogs.title': 'Error Logs',
        'errorlogs.date': 'Date',
        'errorlogs.level': 'Level',
        'errorlogs.source': 'Source',
        'errorlogs.message': 'Message',
        'errorlogs.request': 'Request',
        'errorlogs.user': 'User',
        'errorlogs.exception': 'Exception',
        'errorlogs.exceptionType': 'Type',
        'errorlogs.noException': 'No exception details.',
        'errorlogs.noLogs': 'No errors recorded.',
        'errorlogs.total': 'Total',
        'errorlogs.clear': 'Clear',
        'errorlogs.clearConfirm': 'Delete all error logs? This cannot be undone.',
        'auditlog.event.ProviderCreated': 'Provider Created',
        'auditlog.event.ProviderUpdated': 'Provider Updated',
        'auditlog.event.ProviderDeleted': 'Provider Deleted',
        'auditlog.event.FuelAdded': 'Fuel Added',
        'auditlog.event.FuelRemoved': 'Fuel Removed',
        'auditlog.event.PriceChanged': 'Price Changed',
        'auditlog.event.NominalSetChanged': 'Nominal Set Changed',
        'auditlog.event.AdminLoggedIn': 'Admin Logged In',
        'auditlog.event.AdminLoginFailed': 'Admin Login Failed',
        'auditlog.event.UserDeleted': 'User Deleted',
    },

    uk: {
        // App
        'app.title': 'FUEL FLOW',
        'app.admin': 'АДМІН',

        // Sidebar Navigation
        'nav.management': 'Управління',
        'nav.fueltypes': 'Типи пального',
        'nav.purchases': 'Покупки',
        'nav.users': 'Користувачі',
        'nav.providers': 'Постачальники',
        'nav.vouchers': 'Імпорт талонів',
        'nav.imports': 'Імпорти',
        'nav.contracts': 'Договори',
        'nav.reconciliation': 'Аудит',
        'nav.auditlog': 'Журнал змін',
        'nav.errorlogs': 'Журнал помилок',
        'nav.reports': 'Звіти',
        'nav.settings': 'Налаштування',

        // Settings
        'settings.autoRefund': 'Автоповернення',
        'settings.enableAutoRefund': 'Увімкнути автоповернення',
        'settings.enableAutoRefundHint': 'Після увімкнення частково виконані замовлення автоматично повертають кошти після закінчення пільгового періоду.',
        'settings.gracePeriod': 'Пільговий період (дні)',
        'settings.gracePeriodHint': '7 = один тиждень · 30 = один місяць · 365 = один рік',
        'settings.save': 'Зберегти налаштування',
        'settings.saved': 'Налаштування збережено',
        'settings.disabledNote': 'Автоповернення ВИМКНЕНО. Замовлення залишаються частково виконаними, поки ви не виконаєте їх або не повернете кошти вручну.',
        'settings.qaTitle': 'Тестовий доступ QA',
        'settings.qaWhat': 'Дозволяє спеціальному тестовому акаунту QA входити у продакшн-застосунок для перевірки App Store, використовуючи зарезервований номер телефону та серверний QA-код. Вмикайте лише під час тестування QA і одразу вимикайте після нього.',
        'settings.qaEnable': 'Увімкнути тестовий доступ QA',
        'settings.qaEnableHint': 'Коли увімкнено, акаунт QA може входити за QA-кодом. Коли вимкнено, цей вхід перестає працювати, а активна сесія QA завершується.',
        'settings.qaStatusEnabled': 'Увімкнено',
        'settings.qaStatusDisabled': 'Вимкнено',
        'settings.qaPhone': 'Телефон акаунта QA',
        'settings.qaNotConfigured': 'На сервері не налаштовано QA-код, тож вхід QA не працюватиме навіть після увімкнення. Задайте секрет QA-коду в конфігурації розгортання перед увімкненням.',
        'settings.qaDisabledNote': 'Тестовий доступ QA ВИМКНЕНО. Номер і код QA не надають доступу.',
        'settings.qaLastChanged': 'Останню зміну зроблено',
        'settings.qaSaved': 'Тестовий доступ QA оновлено',

        // Imports
        'imports.title': 'Імпорти талонів',
        'imports.back': 'До списку імпортів',
        'imports.activateAll': 'Активувати всі',
        'imports.deactivateAll': 'Деактивувати всі',
        'imports.number': 'Номер',
        'imports.verification': 'Верифікація',
        'imports.noVouchers': 'Немає ваучерів у цьому імпорті',
        'imports.file': 'Файл',
        'imports.pages': 'Сторінок',
        'imports.vouchers': 'Ваучерів',
        'imports.imported': 'Імпортовано',
        'imports.duplicates': 'Дублікатів',
        'imports.errors': 'Помилок',
        'imports.warnings': 'Попередж.',
        'imports.created': 'Створено',
        'imports.none': 'Ще немає імпортів',
        'status.deactivated': 'Деактивовано',
        'contracts.availableTitle': 'Доступні договори',
        'contracts.name': 'Назва',
        'contracts.version': 'Версія',
        'contracts.createdAt': 'Дата створення',
        'contracts.noneAvailable': 'Додаткових договорів не знайдено',
        'contracts.signedTitle': 'Підписані договори',
        'contracts.user': 'Користувач',
        'contracts.company': 'Компанія',
        'contracts.contract': 'Договір',
        'contracts.signedAt': 'Дата підпису',
        'contracts.signature': 'Підпис',
        'contracts.view': 'Переглянути',
        'contracts.noneSigned': 'Підписаних договорів ще немає',

        // Fuel Prices
        'fuelprices.packagePrices': 'Ціни пакетів (₴/л)',
        'fuelprices.editHint': 'Натисніть на ціну для редагування · Tab / ↑↓ для навігації · Enter для підтвердження',

        // User Profile
        'user.name': 'Адміністратор',
        'user.role': 'Суперкористувач',

        // Header
        'header.dashboard': 'Дашборд',
        'header.docs': 'Документація',
        'header.support': 'Підтримка',

        // Common
        'common.loading': 'Завантаження...',
        'common.save': 'Зберегти',
        'common.cancel': 'Скасувати',
        'common.delete': 'Видалити',
        'common.deleteAll': 'Видалити усі',
        'common.edit': 'Редагувати',
        'common.create': 'Створити',
        'common.search': 'Пошук...',
        'common.actions': 'Дії',
        'common.status': 'Статус',
        'common.date': 'Дата',
        'common.print': 'Друк',
        'common.close': 'Закрити',
        'common.add': 'Додати',
        'common.history': 'Історія',
        'common.noData': 'Немає даних',
        'common.saved': 'Збережено',
        'common.deleted': 'Видалено',
        'common.created': 'Створено',
        'common.confirm': 'Підтвердити',
        'table.nominals': 'Номінали',
        'price.supplier': 'Постачальник',
        'price.margin': 'Маржа',
        'price.final': 'Фінал',
        'price.unit': 'грн/л',
        'price.supplierPlaceholder': 'напр. 50.00',
        'providers.addProvider': 'Додати Постачальника',
        'providers.namePlaceholder': 'Назва (напр. OKKO)',
        'providers.logoText': 'Текст Логотипу',
        'providers.logoTextPlaceholder': 'Текст Логотипу',
        'providers.color': 'Колір',
        'providers.editProvider': 'Редагувати Постачальника',
        'providers.noProviders': 'Постачальників не знайдено',
  'providers.priority': 'Пріоритет',
  'providers.priorityHint': 'Порядок показу в мобільному застосунку. Менше = вище. 999 = кінець списку.',
  'providers.discount': 'Знижка',
  'providers.discountHint': 'Маркетингова знижка (грн/л) проти ціни на стелі — показується клієнту. Не змінює ціну списання.',
        'providers.deleteConfirm': 'Видалити {0}?',
        'providers.nominalsCount': '{0} номіналів',
        'history.providerCreated': 'Створено постачальника {0}',
        'history.providerDeleted': 'Видалено постачальника {0}',
        'history.providerUpdated': 'Оновлено постачальника {0}',
        'history.nominalsChanged': '{0}: номінали змінено [{1}]',
        'history.fuelAdded': '{0} / {1}: додано за {2} грн/л',
        'history.fuelRemoved': '{0} / {1}: видалено',
        'history.fuelUpdated': '{0} / {1}: оновлено',
        'history.changeName': 'назва {0} → {1}',
        'history.changeSupplier': 'закупівельна ціна {0} → {1}',
        'history.changeMargin': 'маржа {0} → {1}',
        'history.changeFinal': 'фінальна ціна {0} → {1}',
        'history.changeLogo': 'логотип {0} → {1}',
        'history.changeColor': 'колір {0} → {1}',
        'price.marginPlaceholder': 'напр. 2.00',
        'price.finalPlaceholder': 'Фінал (авто)',
        'price.nominalsAuto': 'авто',
        'price.title': 'Ціни на пальне',

        // Reports
        'report.allUsers': 'Всі користувачі',
        'report.title': 'Звіти користувачів',
        'report.from': 'Від',
        'report.to': 'До',
        'report.yesterday': 'Вчора',
        'report.30days': '30 днів',
        'report.week': 'Тиждень',
        'report.month': 'Місяць',
        'report.generate': 'Сформувати звіт',
        'report.reconciliationAct': 'Акт звірки',
        'report.loading': 'Завантаження...',
        'report.noData': 'Оберіть параметри та натисніть "Сформувати звіт"',
        'report.totalSpent': 'Прибуток',
        'report.orders': 'замовлень',
        'report.purchased': 'Придбано талонів',
        'report.litersPurchased': '~{0} літрів',
        'report.used': 'Використано талонів',
        'report.litersUsed': 'ВИКОРИСТАНО ЛІТРІВ',
        'report.monthlyBreakdown': 'Помісячна деталізація',
        'report.amount': 'Сума',
        'report.payments': 'Платежі',
        'report.paymentId': 'ID',
        'report.provider': 'Постачальник',
        'report.fuel': 'Паливо',
        'report.amountHeader': 'Сума',
        'report.liters': 'Літри',
        'report.quantity': 'К-сть',
        'report.bank': 'Банк',
        'report.status': 'Статус',
        'report.date': 'Дата',
        'report.noPayments': 'Немає платежів',
        'report.monobankInvoices': 'Monobank інвойси: {0}',
        'report.redemptions': 'Використання талонів',
        'report.redemptionId': 'ID',
        'report.fuelName': 'Паливо',
        'report.redeemedAt': 'Дата використання',
        'report.noRedemptions': 'Немає використаних талонів',
        'report.fulfilledOrders': 'виконаних замовлень',
        'report.profitSummary': 'Зведення прибутку',
        'report.period': 'Період',
        'report.orders_short': 'Замовлень',
        'report.profit': 'Прибуток',
        'report.avgOrder': 'Середнє замовлення',

        // Forms
        'forms.selectStation': 'Оберіть станцію',
        'forms.selectFuel': 'Оберіть пальне',
        'forms.stationIdPlaceholder': 'ID Станції (напр. okko)',
        'forms.stationNamePlaceholder': 'Назва Станції (напр. OKKO)',
        'forms.logoTextPlaceholder': 'Текст Логотипу',
        'forms.fuelIdPlaceholder': 'ID Пального (напр. a95)',
        'forms.fuelNamePlaceholder': 'напр. A-95',
        'forms.basePricePlaceholder': 'Базова ціна',
        'forms.discountPricePlaceholder': 'Ціна зі знижкою',
        'forms.qrUrlPlaceholder': 'URL QR-коду або URL Зображення',
        'forms.packageIdPlaceholder': 'ID Пакету (напр. okko_a95_10)',
        'forms.priceUahPlaceholder': 'Ціна (ГРН)',
        'forms.originalPriceUahPlaceholder': 'Початкова ціна (ГРН)',

        // Dashboard
        'dashboard.availableQrs': 'Доступні QR',
        'dashboard.soldQrs': 'Продані QR',

        // Tables
        'table.id': 'ID',
        'table.name': 'Назва',
        'table.logo': 'Лого',
        'table.color': 'Колір',
        'table.station': 'Станція',
        'table.fuel': 'Пальне',
        'table.liters': 'Літри',
        'table.price': 'Ціна',
        'table.number': '№',
        'table.description': 'Опис',
        'table.quantity': 'К-сть',
        'table.date': 'Дата',
        'table.amount': 'Сума, грн',
        'table.debit': 'Дебет',
        'table.credit': 'Кредит',
        'table.uah': 'грн',
        'table.original': 'Початкова',
        'table.qrPreview': 'Перегляд QR',
        'table.basePrice': 'Базова ціна',
        'table.discountPrice': 'Зі знижкою',
        'table.phone': 'Телефон',
        'table.email': 'Email',
        'table.birthdate': 'Дата народження',
        'table.bonusBalance': 'Бонусний Баланс',
        'table.referralCode': 'Реферальний Код',
        'table.referredBy': 'Запросив',
        'table.role': 'Роль',
        'table.status': 'Статус',
        'users.active': 'Активний',
        'users.inactive': 'Неактивний',
        'users.deleted': 'Видалено',
        'users.createdAt': 'Дата створення',
        'users.activate': 'Активувати',
        'users.deactivate': 'Деактивувати',
        'users.activated': 'Користувача активовано',
        'users.deactivated': 'Користувача деактивовано',
        'users.delete': 'Видалити',
        'users.deleteConfirm': 'Видалити цього користувача? Його обліковий запис буде заблоковано.',
        'users.deleteSuccess': 'Користувача видалено',
        'users.deleteFailed': 'Не вдалося видалити користувача',
        'users.toggleFailed': 'Не вдалося змінити статус користувача',
        'users.banned_status': 'Заблоковано',
        'users.ban': 'Заблокувати',
        'users.unban': 'Розблокувати',
        'users.banConfirm': 'Заблокувати цього користувача? Усі його активні сесії негайно завершаться, і після розблокування йому доведеться увійти знову.',
        'users.banned': 'Користувача заблоковано',
        'users.unbanned': 'Користувача розблоковано',
        'users.banFailed': 'Не вдалося змінити статус блокування',
        'users.roleChanged': 'Роль оновлено',
        'users.roleChangeFailed': 'Не вдалося змінити роль',
        'users.editEmail': 'Змінити email',
        'users.emailPrompt': 'Новий email (залиште порожнім, щоб очистити). На нову адресу буде надіслано посилання для підтвердження.',
        'users.emailChangeSent': 'Посилання для підтвердження надіслано на нову адресу',
        'users.emailCleared': 'Email очищено',
        'users.emailChangeFailed': 'Не вдалося змінити email',

        // Purchases
        'table.vouchers': 'Ваучери',
        'purchases.refund': 'Повернення',
        'purchases.refundTitle': 'Підтвердити повернення',
        'purchases.refundConfirm': 'Повернути кошти за це замовлення? Невиконану частину на суму {0} грн буде повернено клієнту.',
        'purchases.refundNothingToRefund': 'Невиконаної частини замовлення не залишилось.',
        'purchases.refundStatus.Processing': 'Повернення в обробці',
        'purchases.refundStatus.Completed': 'Повернення підтверджено',
        'purchases.refundStatus.Failed': 'Повернення не вдалося',
        'purchases.refundSuccess': 'Повернення ініційовано: {0} грн',
        'purchases.refundFailed': 'Не вдалося виконати повернення',

        // Vouchers / Import
        'import.title': 'Імпорт талонів',
        'import.description': 'Завантажте PDF або зображення для імпорту талонів у систему.',
        'import.dragDrop': 'Перетягніть сюди зображення талонів, PDF або ZIP',
        'import.clickSelect': 'Натисніть, щоб вибрати',
        'import.uploading': 'Завантаження...',
        'import.processing': 'Обробка...',
        'import.largeFileNote': 'Великі файли можуть оброблятися кілька хвилин.',
        'import.completed': 'Завершено',
        'import.failed': 'Помилка',
        'import.totalFiles': 'Всього файлів',
        'import.processed': 'Оброблено',
        'import.successful': 'Успішно',
        'import.failedCount': 'Помилок',
        'import.start': 'Почати імпорт',
        'import.filesSelected': 'Вибрано {0} файл(ів):',
        'import.close': 'Закрити',
        'import.model': 'Модель',
        'import.errorOccurred': 'Сталася помилка під час імпорту.',
        'import.duplicates': 'Дублікати',
        'import.noVouchers': 'Талони ще не імпортовані.',
        'import.scanTitle': 'Сканувати талон',

        // Voucher Table
        'vouchers.id': 'ID',
        'vouchers.image': 'Зображення',
        'vouchers.volume': 'Об\'єм',
        'vouchers.fuelType': 'Тип пального',
        'vouchers.provider': 'Провайдер',
        'vouchers.status': 'Статус',
        'vouchers.importedAt': 'Імпортовано',
        'vouchers.expires': 'Термін дії',
        'vouchers.externalId': 'Зовнішній ID',
        'vouchers.total': 'Всього',
        'vouchers.filtered': 'Відфільтровано',
        'vouchers.selected': 'Вибрано',
        'vouchers.allFuelTypes': 'Усі типи пального',
        'vouchers.allStatuses': 'Усі статуси',
        'vouchers.allProviders': 'Усі провайдери',
        'vouchers.all': 'Усі',
        'vouchers.clearFilters': 'Очистити фільтри',
        'vouchers.delete': 'Видалити',
        'vouchers.deleteConfirmTitle': 'Підтвердження видалення',
        'vouchers.deleteConfirm': 'Ви впевнені, що хочете видалити {0} талонів? Цю дію не можна скасувати.',
        'vouchers.deleteAllTitle': 'Видалити всі талони',
        'vouchers.deleteAllConfirm': 'Ви впевнені, що хочете видалити всі {0} талонів? Цю дію не можна скасувати.',
        'vouchers.deleteAllAction': 'Видалити всі',
        'vouchers.cancel': 'Скасувати',

        // Voucher Status
        'status.imported': 'Імпортовано',
        'status.available': 'Доступний',
        'status.assigned': 'Призначено',
        'status.used': 'Використано',
        'status.sold': 'Продано',
        'status.expired': 'Протерміновано',
        'status.reserved': 'Зарезервовано',
        'status.verificationFailed': 'Помилка верифікації',
        'status.active': 'Активний',
        'status.delivered': 'Доставлено',
        'status.pending': 'Очікує',
        'status.pending_qr': 'Очікує QR',
        'status.completed': 'Завершено',
        'status.failed': 'Помилка',
        'status.started': 'Розпочато',
        'status.verifiedWithWarnings': 'Пройшов із зауваженнями',

        // Order Statuses
        'order.status.pendingPayment': 'Очікує оплати',
        'order.status.paid': 'Оплачено',
        'order.status.pendingFulfillment': 'Очікує виконання',
        'order.status.fulfilled': 'Виконано',
        'order.status.partiallyFulfilled': 'Частково виконано',
        'order.status.partiallyRefunded': 'Частково повернуто',
        'order.status.pending': 'Очікує',
        'order.status.cancelled': 'Скасовано',
        'order.status.refunded': 'Повернено',

        // Monobank
        'monobank.status.success': 'Успішно',
        'monobank.status.pending': 'Очікує',

        // Reconciliation
        'reconciliation.title': 'Аудит',
        'reconciliation.summary': 'Зведення',
        'reconciliation.revenue': 'Дохід',
        'reconciliation.fulfilled': 'Виконано',
        'reconciliation.pending': 'Очікує',
        'reconciliation.unprocessed': 'Необроблені події',
        'reconciliation.importErrors': 'Помилки імпорту (7д)',
        'reconciliation.pendingOrders': 'Замовлення в очікуванні',
        'reconciliation.voucherInventory': 'Інвентаризація талонів',
        'reconciliation.monthlyRevenue': 'Щомісячний дохід',
        'reconciliation.recentImports': 'Останні імпорти',
        'reconciliation.noPending': 'Немає замовлень в очікуванні',
        'reconciliation.noImports': 'Немає останніх імпортів',
        'reconciliation.modalTitle': 'Акт звірки взаєморозрахунків',
        'reconciliation.documentTitle': 'АКТ ЗВІРКИ ВЗАЄМОРОЗРАХУНКІВ',
        'reconciliation.periodLabel': 'Період:',
        'reconciliation.periodFrom': 'з початку',
        'reconciliation.periodTo': 'по сьогодні',
        'reconciliation.userLabel': 'Користувач:',
        'reconciliation.noOperations': 'Немає операцій за обраний період',
        'reconciliation.paymentDesc': 'Придбання талонів: {0} {1}L x{2}',
        'reconciliation.redemptionDesc': 'Використання талону: {0} {1}L',
        'reconciliation.total': 'Всього:',
        'reconciliation.balance': 'Сальдо кінцеве:',
        'reconciliation.summaryInfo': 'Зведена інформація:',
        'reconciliation.totalOrders': 'Всього замовлень: {0}',
        'reconciliation.vouchersPurchased': 'Придбано талонів: {0} шт. ({1} л)',
        'reconciliation.vouchersUsed': 'Використано талонів: {0} шт. ({1} л)',
        'reconciliation.fuel': 'паливо',
        'reconciliation.adminSignature': 'Підпис адміністратора:',
        'reconciliation.userSignature': 'Підпис користувача:',
        'reconciliation.needAttention': 'Потребує уваги',
        'reconciliation.unfulfilled': 'невиконано',
        'reconciliation.partial': 'частково',
        'reconciliation.dataIntegrity': 'Цілісність даних',
        'reconciliation.orphanVouchers': 'сироти талонів / необроблені події',
        'reconciliation.providersDeficit': 'Постачальники з дефіцитом',
        'reconciliation.importErrors7d': 'помилки імпорту (7д)',
        'reconciliation.issuesFound': 'Знайдено проблем ({0})',
        'reconciliation.critical': 'КРИТИЧНО',
        'reconciliation.warning': 'ПОПЕРЕДЖЕННЯ',
        'reconciliation.threeWayMatch': 'Тристороннє зіставлення · Замовлення — Оплата — Виконання',
        'reconciliation.threeOrder': 'Замовлення',
        'reconciliation.threePayment': 'Оплата',
        'reconciliation.threeFulfillment': 'Виконання',
        'reconciliation.threeVouchExp': 'Очік. Тал',
        'reconciliation.threeVouchDel': 'Дост. Тал',
        'reconciliation.threeMatch': 'Збіг',
        'reconciliation.threeAge': 'Вік',
        'reconciliation.matchOk': 'OK',
        'reconciliation.matchPartial': 'ЧАСТКОВО',
        'reconciliation.matchUnfulfilled': 'НЕВИКОНАНО',
        'reconciliation.matchCancelled': 'СКАСОВАНО',
        'reconciliation.matchRefunded': 'ПОВЕРНЕНО',
        'reconciliation.matchPartial_refunded': 'ЧАСТКОВО + ПОВЕРНЕННЯ',
        'reconciliation.threeRefund': 'Повернення',
        'reconciliation.moneyLedger': 'Грошовий облік · отримано — виконано — повернено',
        'reconciliation.received': 'Отримано',
        'reconciliation.deliveredValue': 'Виконана вартість',
        'reconciliation.refundedLabel': 'Повернено',
        'reconciliation.refundedOrders': 'повернених замовлень',
        'reconciliation.refundDesc': 'Повернення за неотримані талони: {0}',
        'reconciliation.noOrderData': 'Немає даних замовлень',
        'reconciliation.voucherFunnel': 'Воронка талонів',
        'reconciliation.loadFailed': 'Не вдалося завантажити дані',

        // Audit Log
        'auditlog.title': 'Журнал змін',
        'auditlog.eventType': 'Подія',
        'auditlog.aggregate': 'Сутність',
        'auditlog.changedBy': 'Хто змінив',
        'auditlog.date': 'Дата',
        'auditlog.summary': 'Опис',
        'auditlog.noEvents': 'Події ще не записані.',
        'auditlog.total': 'Всього',

        // Error Logs
        'errorlogs.title': 'Журнал помилок',
        'errorlogs.date': 'Дата',
        'errorlogs.level': 'Рівень',
        'errorlogs.source': 'Джерело',
        'errorlogs.message': 'Повідомлення',
        'errorlogs.request': 'Запит',
        'errorlogs.user': 'Користувач',
        'errorlogs.exception': 'Виняток',
        'errorlogs.exceptionType': 'Тип',
        'errorlogs.noException': 'Немає деталей винятку.',
        'errorlogs.noLogs': 'Помилки не записані.',
        'errorlogs.total': 'Всього',
        'errorlogs.clear': 'Очистити',
        'errorlogs.clearConfirm': 'Видалити всі записи помилок? Цю дію не можна скасувати.',
        'auditlog.event.ProviderCreated': 'Постачальника створено',
        'auditlog.event.ProviderUpdated': 'Постачальника оновлено',
        'auditlog.event.ProviderDeleted': 'Постачальника видалено',
        'auditlog.event.FuelAdded': 'Пальне додано',
        'auditlog.event.FuelRemoved': 'Пальне видалено',
        'auditlog.event.PriceChanged': 'Ціну змінено',
        'auditlog.event.NominalSetChanged': 'Номінали змінено',
        'auditlog.event.AdminLoggedIn': 'Вхід адміністратора',
        'auditlog.event.AdminLoginFailed': 'Невдалий вхід адміністратора',
        'auditlog.event.UserDeleted': 'Користувача видалено',
    },

    de: {
        // App
        'app.title': 'FUEL FLOW',
        'app.admin': 'ADMIN',

        // Sidebar Navigation
        'nav.management': 'Verwaltung',
        'nav.fueltypes': 'Kraftstoffarten',
        'nav.purchases': 'Einkäufe',
        'nav.users': 'Benutzer',
        'nav.providers': 'Anbieter',
        'nav.vouchers': 'Gutscheine Importieren',
        'nav.imports': 'Importe',
        'nav.contracts': 'Verträge',
        'nav.reconciliation': 'Audit',
        'nav.auditlog': 'Audit-Log',
        'nav.errorlogs': 'Fehlerprotokoll',
        'nav.reports': 'Berichte',
        'nav.settings': 'Einstellungen',

        // Settings
        'settings.autoRefund': 'Automatische Erstattung',
        'settings.enableAutoRefund': 'Automatische Erstattung aktivieren',
        'settings.enableAutoRefundHint': 'Wenn aktiviert, werden teilweise erfüllte Bestellungen nach Ablauf der Wartefrist automatisch erstattet.',
        'settings.gracePeriod': 'Wartefrist (Tage)',
        'settings.gracePeriodHint': '7 = eine Woche · 30 = ein Monat · 365 = ein Jahr',
        'settings.save': 'Einstellungen speichern',
        'settings.saved': 'Einstellungen gespeichert',
        'settings.disabledNote': 'Automatische Erstattung ist AUS. Bestellungen bleiben teilweise erfüllt, bis Sie sie erfüllen oder manuell erstatten.',
        'settings.qaTitle': 'QA-Testzugang',
        'settings.qaWhat': 'Ermöglicht dem speziellen QA-Testkonto die Anmeldung in der Produktions-App für die App-Store-Prüfung – mit seiner reservierten Telefonnummer und einem serverseitigen QA-Code. Nur während der QA aktivieren und danach sofort deaktivieren.',
        'settings.qaEnable': 'QA-Testzugang aktivieren',
        'settings.qaEnableHint': 'Wenn aktiviert, kann sich das QA-Konto mit seinem QA-Code anmelden. Wenn deaktiviert, funktioniert diese Anmeldung nicht mehr und eine aktive QA-Sitzung wird abgemeldet.',
        'settings.qaStatusEnabled': 'Aktiviert',
        'settings.qaStatusDisabled': 'Deaktiviert',
        'settings.qaPhone': 'Telefon des QA-Kontos',
        'settings.qaNotConfigured': 'Auf dem Server ist kein QA-Code konfiguriert, daher funktioniert die QA-Anmeldung auch bei Aktivierung nicht. Legen Sie das QA-Code-Geheimnis im Deployment fest, bevor Sie aktivieren.',
        'settings.qaDisabledNote': 'QA-Testzugang ist AUS. Die QA-Telefonnummer und der Code gewähren keinen Zugang.',
        'settings.qaLastChanged': 'Zuletzt geändert',
        'settings.qaSaved': 'QA-Testzugang aktualisiert',

        // Imports
        'imports.title': 'Gutscheine Importieren',

        // Fuel Prices
        'fuelprices.packagePrices': 'Paketpreise (₴/L)',
        'fuelprices.editHint': 'Klicken Sie auf einen Preis zum Bearbeiten · Tab / ↑↓ zum Navigieren · Enter zum Bestätigen',

        // User Profile
        'user.name': 'Admin-System',
        'user.role': 'Superuser',

        // Header
        'header.dashboard': 'Dashboard',
        'header.docs': 'Dokumentation',
        'header.support': 'Support',

        // Common
        'common.loading': 'Laden...',
        'common.save': 'Speichern',
        'common.cancel': 'Abbrechen',
        'common.delete': 'Löschen',
        'common.deleteAll': 'Alle löschen',
        'common.edit': 'Bearbeiten',
        'common.create': 'Erstellen',
        'common.search': 'Suchen...',
        'common.actions': 'Aktionen',
        'common.status': 'Status',
        'common.date': 'Datum',
        'common.print': 'Drucken',
        'common.close': 'Schließen',
        'common.add': 'Hinzufügen',
        'common.history': 'Verlauf',
        'common.noData': 'Keine Daten',
        'common.saved': 'Gespeichert',
        'common.deleted': 'Gelöscht',
        'common.created': 'Erstellt',
        'common.confirm': 'Bestätigen',
        'table.nominals': 'Nennwerte',
        'price.supplier': 'Lieferant',
        'price.margin': 'Marge',
        'price.final': 'Endpreis',
        'price.unit': 'UAH/L',
        'price.supplierPlaceholder': 'z.B. 50.00',
        'price.marginPlaceholder': 'z.B. 2.00',
        'price.finalPlaceholder': 'Ende (auto)',
        'price.nominalsAuto': 'auto',
        'providers.addProvider': 'Anbieter Hinzufügen',
        'providers.namePlaceholder': 'Name (z.B. OKKO)',
        'providers.logoText': 'Logo-Text',
        'providers.logoTextPlaceholder': 'Logo-Text',
        'providers.color': 'Farbe',
        'providers.editProvider': 'Anbieter Bearbeiten',
        'providers.noProviders': 'Keine Anbieter gefunden',
  'providers.priority': 'Priorität',
  'providers.priorityHint': 'Anzeigereihenfolge in der mobilen App. Niedriger = höher. 999 = Ende der Liste.',
  'providers.discount': 'Rabatt',
  'providers.discountHint': 'Marketing-Rabatt (UAH/l) gegenüber der Zapfsäulen-Preises - wird dem Kunden angezeigt. Ändert den Abrechnungspreis nicht.',
        'providers.deleteConfirm': '{0} löschen?',
        'providers.nominalsCount': '{0} Nennwerte',
        'history.providerCreated': 'Anbieter erstellt {0}',
        'history.providerDeleted': 'Anbieter gelöscht {0}',
        'history.providerUpdated': 'Anbieter aktualisiert {0}',
        'history.nominalsChanged': '{0}: Nennwerte geändert [{1}]',
        'history.fuelAdded': '{0} / {1}: hinzugefügt für {2} UAH/L',
        'history.fuelRemoved': '{0} / {1}: entfernt',
        'history.fuelUpdated': '{0} / {1}: aktualisiert',
        'history.changeName': 'Name {0} → {1}',
        'history.changeSupplier': 'Lieferpreis {0} → {1}',
        'history.changeMargin': 'Marge {0} → {1}',
        'history.changeFinal': 'Endpreis {0} → {1}',
        'history.changeLogo': 'Logo {0} → {1}',
        'history.changeColor': 'Farbe {0} → {1}',
        'price.title': 'Kraftstoffpreise',

        // Reports
        'report.allUsers': 'Alle Benutzer',
        'report.title': 'Benutzerberichte',
        'report.from': 'Von',
        'report.to': 'Bis',
        'report.yesterday': 'Gestern',
        'report.30days': '30 Tage',
        'report.week': 'Woche',
        'report.month': 'Monat',
        'report.generate': 'Bericht erstellen',
        'report.reconciliationAct': 'Abstimmungsakt',
        'report.loading': 'Laden...',
        'report.noData': 'Wählen Sie Parameter und klicken Sie auf "Bericht erstellen"',
        'report.totalSpent': 'Gewinn',
        'report.orders': 'Bestellungen',
        'report.purchased': 'Erworbene Gutscheine',
        'report.litersPurchased': '~{0} Liter',
        'report.used': 'Verwendete Gutscheine',
        'report.litersUsed': 'LITER GENUTZT',
        'report.monthlyBreakdown': 'Monatliche Aufschlüsselung',
        'report.amount': 'Betrag',
        'report.payments': 'Zahlungen',
        'report.paymentId': 'ID',
        'report.provider': 'Anbieter',
        'report.fuel': 'Kraftstoff',
        'report.amountHeader': 'Betrag',
        'report.liters': 'Liter',
        'report.quantity': 'Anz.',
        'report.bank': 'Bank',
        'report.status': 'Status',
        'report.date': 'Datum',
        'report.noPayments': 'Keine Zahlungen',
        'report.monobankInvoices': 'Monobank-Rechnungen: {0}',
        'report.redemptions': 'Gutscheineinlösungen',
        'report.redemptionId': 'ID',
        'report.fuelName': 'Kraftstoff',
        'report.redeemedAt': 'Eingelöst am',
        'report.noRedemptions': 'Keine eingelösten Gutscheine',
        'report.fulfilledOrders': 'erfüllte Bestellungen',
        'report.profitSummary': 'Gewinnübersicht',
        'report.period': 'Zeitraum',
        'report.orders_short': 'Bestellungen',
        'report.profit': 'Gewinn',
        'report.avgOrder': 'Ø Bestellung',

        // Forms
        'forms.selectStation': 'Station auswählen',
        'forms.selectFuel': 'Kraftstoff auswählen',
        'forms.stationIdPlaceholder': 'Stations-ID (z.B. okko)',
        'forms.stationNamePlaceholder': 'Stationsname (z.B. OKKO)',
        'forms.logoTextPlaceholder': 'Logo-Text',
        'forms.fuelIdPlaceholder': 'Kraftstoff-ID (z.B. a95)',
        'forms.fuelNamePlaceholder': 'z.B. A-95',
        'forms.basePricePlaceholder': 'Grundpreis',
        'forms.discountPricePlaceholder': 'Rabattpreis',
        'forms.qrUrlPlaceholder': 'QR-Code-URL oder Bild-URL',
        'forms.packageIdPlaceholder': 'Paket-ID (z.B. okko_a95_10)',
        'forms.priceUahPlaceholder': 'Preis (UAH)',
        'forms.originalPriceUahPlaceholder': 'Originalpreis (UAH)',

        // Dashboard
        'dashboard.availableQrs': 'Verfügbare QRs',
        'dashboard.soldQrs': 'Verkaufte QRs',

        // Tables
        'table.id': 'ID',
        'table.name': 'Name',
        'table.logo': 'Logo',
        'table.color': 'Farbe',
        'table.station': 'Station',
        'table.fuel': 'Kraftstoff',
        'table.liters': 'Liter',
        'table.price': 'Preis',
        'table.number': 'Nr.',
        'table.description': 'Beschreibung',
        'table.quantity': 'Menge',
        'table.date': 'Datum',
        'table.amount': 'Betrag, UAH',
        'table.debit': 'Soll',
        'table.credit': 'Haben',
        'table.uah': 'UAH',
        'table.original': 'Original',
        'table.qrPreview': 'QR-Vorschau',
        'table.basePrice': 'Grundpreis',
        'table.discountPrice': 'Rabattpreis',
        'table.phone': 'Telefon',
        'table.email': 'E-Mail',
        'table.birthdate': 'Geburtsdatum',
        'table.bonusBalance': 'Bonusguthaben',
        'table.referralCode': 'Empfehlungscode',
        'table.referredBy': 'Empfohlen von',
        'table.role': 'Rolle',
        'table.status': 'Status',
        'users.active': 'Aktiv',
        'users.inactive': 'Inaktiv',
        'users.deleted': 'Gelöscht',
        'users.createdAt': 'Erstellt am',
        'users.activate': 'Aktivieren',
        'users.deactivate': 'Deaktivieren',
        'users.activated': 'Benutzer aktiviert',
        'users.deactivated': 'Benutzer deaktiviert',
        'users.delete': 'Löschen',
        'users.deleteConfirm': 'Diesen Benutzer löschen? Sein Konto wird dauerhaft gesperrt.',
        'users.deleteSuccess': 'Benutzer gelöscht',
        'users.deleteFailed': 'Fehler beim Löschen des Benutzers',
        'users.toggleFailed': 'Fehler beim Ändern des Benutzerstatus',
        'users.banned_status': 'Gesperrt',
        'users.ban': 'Sperren',
        'users.unban': 'Entsperren',
        'users.banConfirm': 'Diesen Benutzer sperren? Alle aktiven Sitzungen enden sofort und er muss sich nach dem Entsperren erneut anmelden.',
        'users.banned': 'Benutzer gesperrt',
        'users.unbanned': 'Benutzer entsperrt',
        'users.banFailed': 'Fehler beim Ändern des Sperrstatus',
        'users.roleChanged': 'Rolle aktualisiert',
        'users.roleChangeFailed': 'Rolle konnte nicht geändert werden',
        'users.editEmail': 'E-Mail bearbeiten',
        'users.emailPrompt': 'Neue E-Mail (leer lassen zum Entfernen). An die neue Adresse wird zuerst ein Bestätigungslink gesendet.',
        'users.emailChangeSent': 'Bestätigungslink an die neue Adresse gesendet',
        'users.emailCleared': 'E-Mail entfernt',
        'users.emailChangeFailed': 'E-Mail konnte nicht geändert werden',

        // Purchases
        'table.vouchers': 'Gutscheine',
        'purchases.refund': 'Erstattung',
        'purchases.refundTitle': 'Erstattung bestätigen',
        'purchases.refundConfirm': 'Diese Bestellung erstatten? Nicht erfüllter Wert von {0} UAH wird an den Kunden zurückgezahlt.',
        'purchases.refundNothingToRefund': 'Es verbleibt kein nicht erfüllter Wert für diese Bestellung.',
        'purchases.refundStatus.Processing': 'Erstattung ausstehend',
        'purchases.refundStatus.Completed': 'Erstattung bestätigt',
        'purchases.refundStatus.Failed': 'Erstattung fehlgeschlagen',
        'purchases.refundSuccess': 'Erstattung eingeleitet: {0} UAH',
        'purchases.refundFailed': 'Erstattung fehlgeschlagen',

        // Vouchers / Import
        'import.title': 'Gutscheine Importieren',
        'import.description': 'Laden Sie PDF- oder Bilddateien hoch, um Gutscheine in das System zu importieren.',
        'import.dragDrop': 'Gutscheinbilder, PDFs oder ZIP hierher ziehen',
        'import.clickSelect': 'Klicken zum Auswählen',
        'import.uploading': 'Hochladen...',
        'import.processing': 'Verarbeiten...',
        'import.largeFileNote': 'Große Dateien können einige Minuten dauern.',
        'import.filesSelected': 'Ausgewählt: {0} Datei(en):',
        'import.completed': 'Abgeschlossen',
        'import.failed': 'Fehlgeschlagen',
        'import.totalFiles': 'Gesamtdateien',
        'import.processed': 'Verarbeitet',
        'import.successful': 'Erfolgreich',
        'import.failedCount': 'Fehlgeschlagen',

        // Voucher Table
        'vouchers.id': 'ID',
        'vouchers.image': 'Bild',
        'vouchers.volume': 'Volumen',
        'vouchers.fuelType': 'Kraftstoffart',
        'vouchers.provider': 'Anbieter',
        'vouchers.status': 'Status',
        'vouchers.importedAt': 'Importiert am',
        'vouchers.expires': 'Ablaufdatum',
        'vouchers.externalId': 'Externe ID',
        'vouchers.total': 'Gesamt',
        'vouchers.filtered': 'Gefiltert',
        'vouchers.selected': 'Ausgewählt',
        'vouchers.allFuelTypes': 'Alle Kraftstoffarten',
        'vouchers.allStatuses': 'Alle Status',
        'vouchers.allProviders': 'Alle Anbieter',
        'vouchers.all': 'Alle',
        'vouchers.clearFilters': 'Filter zurücksetzen',
        'vouchers.delete': 'Löschen',
        'vouchers.deleteConfirmTitle': 'Löschen bestätigen',
        'vouchers.deleteConfirm': 'Möchten Sie wirklich {0} Gutscheine löschen? Diese Aktion kann nicht rückgängig gemacht werden.',
        'vouchers.deleteAllTitle': 'Alle Gutscheine löschen',
        'vouchers.deleteAllConfirm': 'Möchten Sie wirklich alle {0} Gutscheine löschen? Diese Aktion kann nicht rückgängig gemacht werden.',
        'vouchers.deleteAllAction': 'Alle löschen',
        'vouchers.cancel': 'Abbrechen',

        // Voucher Status
        'status.imported': 'Importiert',
        'status.available': 'Verfügbar',
        'status.assigned': 'Zugewiesen',
        'status.used': 'Verwendet',
        'status.sold': 'Verkauft',
        'status.expired': 'Abgelaufen',
        'status.reserved': 'Reserviert',
        'status.verificationFailed': 'Verifizierung fehlgeschlagen',
        'status.active': 'Aktiv',
        'status.delivered': 'Geliefert',
        'status.pending': 'Ausstehend',
        'status.pending_qr': 'QR ausstehend',
        'status.completed': 'Abgeschlossen',
        'status.failed': 'Fehlgeschlagen',
        'status.started': 'Gestartet',
        'status.verifiedWithWarnings': 'Mit Warnungen verifiziert',

        // Order Statuses
        'order.status.pendingPayment': 'Ausstehende Zahlung',
        'order.status.paid': 'Bezahlt',
        'order.status.pendingFulfillment': 'Wartet auf Erfüllung',
        'order.status.fulfilled': 'Erfüllt',
        'order.status.partiallyFulfilled': 'Teilweise erfüllt',
        'order.status.partiallyRefunded': 'Teilweise erstattet',
        'order.status.pending': 'Ausstehend',
        'order.status.cancelled': 'Storniert',
        'order.status.refunded': 'Erstattet',

        // Monobank
        'monobank.status.success': 'Erfolg',
        'monobank.status.pending': 'Ausstehend',

        // Reconciliation
        'reconciliation.title': 'Audit',
        'reconciliation.modalTitle': 'Abstimmungsakt',
        'reconciliation.documentTitle': 'ABSTIMMUNGSAKT',
        'reconciliation.periodLabel': 'Zeitraum:',
        'reconciliation.periodFrom': 'von Anfang an',
        'reconciliation.periodTo': 'bis heute',
        'reconciliation.userLabel': 'Benutzer:',
        'reconciliation.noOperations': 'Keine Vorgänge im ausgewählten Zeitraum',
        'reconciliation.paymentDesc': 'Gutscheinkauf: {0} {1}L x{2}',
        'reconciliation.redemptionDesc': 'Gutscheineinlösung: {0} {1}L',
        'reconciliation.total': 'Gesamt:',
        'reconciliation.balance': 'Endsaldo:',
        'reconciliation.summaryInfo': 'Zusammenfassung:',
        'reconciliation.totalOrders': 'Bestellungen insgesamt: {0}',
        'reconciliation.vouchersPurchased': 'Gutscheine gekauft: {0} Stk. ({1} L)',
        'reconciliation.vouchersUsed': 'Gutscheine verwendet: {0} Stk. ({1} L)',
        'reconciliation.fuel': 'Kraftstoff',
        'reconciliation.adminSignature': 'Administrator-Unterschrift:',
        'reconciliation.userSignature': 'Benutzer-Unterschrift:',
        'reconciliation.needAttention': 'Benötigt Aufmerksamkeit',
        'reconciliation.unfulfilled': 'unerfüllt',
        'reconciliation.partial': 'teilweise',
        'reconciliation.dataIntegrity': 'Datenintegrität',
        'reconciliation.orphanVouchers': 'verwaiste Gutscheine / unverarbeitete Ereignisse',
        'reconciliation.providersDeficit': 'Anbieter mit Defizit',
        'reconciliation.importErrors7d': 'Importfehler (7d)',
        'reconciliation.issuesFound': 'Probleme gefunden ({0})',
        'reconciliation.critical': 'KRITISCH',
        'reconciliation.warning': 'WARNUNG',
        'reconciliation.threeWayMatch': 'Drei-Wege-Abgleich · Bestellung — Zahlung — Erfüllung',
        'reconciliation.threeOrder': 'Bestellung',
        'reconciliation.threePayment': 'Zahlung',
        'reconciliation.threeFulfillment': 'Erfüllung',
        'reconciliation.threeVouchExp': 'Erw. Gutscheine',
        'reconciliation.threeVouchDel': 'Gelief. Gutscheine',
        'reconciliation.threeMatch': 'Abgleich',
        'reconciliation.threeAge': 'Alter',
        'reconciliation.matchOk': 'OK',
        'reconciliation.matchPartial': 'TEILWEISE',
        'reconciliation.matchUnfulfilled': 'UNERFÜLLT',
        'reconciliation.matchCancelled': 'STORNIERT',
        'reconciliation.matchRefunded': 'ERSTATTET',
        'reconciliation.matchPartial_refunded': 'TEILWEISE + ERSTATTUNG',
        'reconciliation.threeRefund': 'Erstattung',
        'reconciliation.moneyLedger': 'Geldbuch · erhalten — geliefert — erstattet',
        'reconciliation.received': 'Erhalten',
        'reconciliation.deliveredValue': 'Gelieferter Wert',
        'reconciliation.refundedLabel': 'Erstattet',
        'reconciliation.refundedOrders': 'erstattete Bestellungen',
        'reconciliation.refundDesc': 'Rückerstattung nicht gelieferter Gutscheine: {0}',
        'reconciliation.noOrderData': 'Keine Bestelldaten',
        'reconciliation.voucherFunnel': 'Gutschein-Trichter',
        'reconciliation.loadFailed': 'Fehler beim Laden der Abstimmungsdaten',

        // Audit Log
        'auditlog.title': 'Audit-Log',
        'auditlog.eventType': 'Ereignis',
        'auditlog.aggregate': 'Entität',
        'auditlog.changedBy': 'Geändert von',
        'auditlog.date': 'Datum',
        'auditlog.summary': 'Beschreibung',
        'auditlog.noEvents': 'Noch keine Ereignisse aufgezeichnet.',
        'auditlog.total': 'Gesamt',

        // Error Logs
        'errorlogs.title': 'Fehlerprotokoll',
        'errorlogs.date': 'Datum',
        'errorlogs.level': 'Schweregrad',
        'errorlogs.source': 'Quelle',
        'errorlogs.message': 'Nachricht',
        'errorlogs.request': 'Anfrage',
        'errorlogs.user': 'Benutzer',
        'errorlogs.exception': 'Ausnahme',
        'errorlogs.exceptionType': 'Typ',
        'errorlogs.noException': 'Keine Ausnahmedetails.',
        'errorlogs.noLogs': 'Keine Fehler aufgezeichnet.',
        'errorlogs.total': 'Gesamt',
        'errorlogs.clear': 'Leeren',
        'errorlogs.clearConfirm': 'Alle Fehlerprotokolle löschen? Dies kann nicht rückgängig gemacht werden.',
        'auditlog.event.ProviderCreated': 'Anbieter erstellt',
        'auditlog.event.ProviderUpdated': 'Anbieter aktualisiert',
        'auditlog.event.ProviderDeleted': 'Anbieter gelöscht',
        'auditlog.event.FuelAdded': 'Kraftstoff hinzugefügt',
        'auditlog.event.FuelRemoved': 'Kraftstoff entfernt',
        'auditlog.event.PriceChanged': 'Preis geändert',
        'auditlog.event.NominalSetChanged': 'Nennwerte geändert',
        'auditlog.event.AdminLoggedIn': 'Admin angemeldet',
        'auditlog.event.AdminLoginFailed': 'Admin-Anmeldung fehlgeschlagen',
        'auditlog.event.UserDeleted': 'Benutzer gelöscht',
    },

    es: {
        // App
        'app.title': 'FUEL FLOW',
        'app.admin': 'ADMIN',

        // Sidebar Navigation
        'nav.management': 'Gestión',
        'nav.fueltypes': 'Tipos de Combustible',
        'nav.purchases': 'Compras',
        'nav.users': 'Usuarios',
        'nav.providers': 'Proveedores',
        'nav.vouchers': 'Importar Cupones',
        'nav.imports': 'Importaciones',
        'nav.contracts': 'Contratos',
        'nav.reconciliation': 'Auditoría',
        'nav.auditlog': 'Registro de Auditoría',
        'nav.errorlogs': 'Registro de Errores',
        'nav.reports': 'Informes',
        'nav.settings': 'Configuración',

        // Settings
        'settings.autoRefund': 'Reembolso automático',
        'settings.enableAutoRefund': 'Activar reembolso automático',
        'settings.enableAutoRefundHint': 'Cuando está activado, los pedidos parcialmente cumplidos se reembolsan automáticamente una vez transcurrido el periodo de gracia.',
        'settings.gracePeriod': 'Periodo de gracia (días)',
        'settings.gracePeriodHint': '7 = una semana · 30 = un mes · 365 = un año',
        'settings.save': 'Guardar configuración',
        'settings.saved': 'Configuración guardada',
        'settings.disabledNote': 'El reembolso automático está DESACTIVADO. Los pedidos permanecen parcialmente cumplidos hasta que los cumplas o los reembolses manualmente.',
        'settings.qaTitle': 'Acceso de prueba QA',
        'settings.qaWhat': 'Permite que la cuenta de prueba QA inicie sesión en la app de producción para la revisión de App Store, usando su número reservado y un código QA del servidor. Actívalo solo mientras dure la QA y desactívalo justo después.',
        'settings.qaEnable': 'Activar acceso de prueba QA',
        'settings.qaEnableHint': 'Cuando está activado, la cuenta QA puede iniciar sesión con su código QA. Cuando está desactivado, ese inicio de sesión deja de funcionar y cualquier sesión QA activa se cierra.',
        'settings.qaStatusEnabled': 'Activado',
        'settings.qaStatusDisabled': 'Desactivado',
        'settings.qaPhone': 'Teléfono de la cuenta QA',
        'settings.qaNotConfigured': 'No hay un código QA configurado en el servidor, por lo que el inicio de sesión QA no funcionará ni siquiera activado. Configura el secreto del código QA en el despliegue antes de activar.',
        'settings.qaDisabledNote': 'El acceso de prueba QA está DESACTIVADO. El teléfono y el código QA no otorgan acceso.',
        'settings.qaLastChanged': 'Último cambio',
        'settings.qaSaved': 'Acceso de prueba QA actualizado',

        // Imports
        'imports.title': 'Importar Cupones',

        // Fuel Prices
        'fuelprices.packagePrices': 'Precios de Paquetes (₴/L)',
        'fuelprices.editHint': 'Haga clic en cualquier precio para editar · Tab / ↑↓ para navegar · Enter para confirmar',

        // User Profile
        'user.name': 'Sistema Admin',
        'user.role': 'Superusuario',

        // Header
        'header.dashboard': 'Panel',
        'header.docs': 'Documentación',
        'header.support': 'Soporte',

        // Common
        'common.loading': 'Cargando...',
        'common.save': 'Guardar',
        'common.cancel': 'Cancelar',
        'common.delete': 'Eliminar',
        'common.deleteAll': 'Eliminar todo',
        'common.edit': 'Editar',
        'common.create': 'Crear',
        'common.search': 'Buscar...',
        'common.actions': 'Acciones',
        'common.status': 'Estado',
        'common.date': 'Fecha',
        'common.print': 'Imprimir',
        'common.close': 'Cerrar',
        'common.add': 'Agregar',
        'common.history': 'Historial',
        'common.noData': 'Sin datos',
        'common.saved': 'Guardado',
        'common.deleted': 'Eliminado',
        'common.created': 'Creado',
        'common.confirm': 'Confirmar',
        'table.nominals': 'Nominales',
        'price.supplier': 'Proveedor',
        'price.margin': 'Margen',
        'price.final': 'Final',
        'price.unit': 'UAH/L',
        'price.supplierPlaceholder': 'ej. 50.00',
        'price.marginPlaceholder': 'ej. 2.00',
        'price.finalPlaceholder': 'Final (auto)',
        'price.nominalsAuto': 'auto',
        'providers.addProvider': 'Agregar Proveedor',
        'providers.namePlaceholder': 'Nombre (ej. OKKO)',
        'providers.logoText': 'Texto del Logo',
        'providers.logoTextPlaceholder': 'Texto del Logo',
        'providers.color': 'Color',
        'providers.editProvider': 'Editar Proveedor',
        'providers.noProviders': 'No se encontraron proveedores',
  'providers.priority': 'Prioridad',
  'providers.priorityHint': 'Orden de visualización en la app móvil. Menor = más alto. 999 = final de la lista.',
  'providers.discount': 'Descuento',
  'providers.discountHint': 'Descuento de marketing (UAH/l) frente al precio de surtidor: se muestra al cliente. No cambia el precio facturado.',
        'providers.deleteConfirm': '¿Eliminar {0}?',
        'providers.nominalsCount': '{0} nominales',
        'history.providerCreated': 'Proveedor creado {0}',
        'history.providerDeleted': 'Proveedor eliminado {0}',
        'history.providerUpdated': 'Proveedor actualizado {0}',
        'history.nominalsChanged': '{0}: nominales cambiados [{1}]',
        'history.fuelAdded': '{0} / {1}: añadido a {2} UAH/L',
        'history.fuelRemoved': '{0} / {1}: eliminado',
        'history.fuelUpdated': '{0} / {1}: actualizado',
        'history.changeName': 'nombre {0} → {1}',
        'history.changeSupplier': 'precio proveedor {0} → {1}',
        'history.changeMargin': 'margen {0} → {1}',
        'history.changeFinal': 'precio final {0} → {1}',
        'history.changeLogo': 'logo {0} → {1}',
        'history.changeColor': 'color {0} → {1}',
        'price.title': 'Precios de Combustible',

        // Reports
        'report.allUsers': 'Todos los Usuarios',
        'report.title': 'Informes de Usuario',
        'report.from': 'Desde',
        'report.to': 'Hasta',
        'report.yesterday': 'Ayer',
        'report.30days': '30 Días',
        'report.week': 'Semana',
        'report.month': 'Mes',
        'report.generate': 'Generar Informe',
        'report.reconciliationAct': 'Acta de Conciliación',
        'report.loading': 'Cargando...',
        'report.noData': 'Seleccione parámetros y haga clic en "Generar Informe"',
        'report.totalSpent': 'Ganancia',
        'report.orders': 'pedidos',
        'report.purchased': 'Comprados',
        'report.litersPurchased': '~{0} litros',
        'report.used': 'Usados',
        'report.litersUsed': 'LITROS USADOS',
        'report.monthlyBreakdown': 'Desglose Mensual',
        'report.amount': 'Importe',
        'report.payments': 'Pagos',
        'report.paymentId': 'ID',
        'report.provider': 'Proveedor',
        'report.fuel': 'Combustible',
        'report.amountHeader': 'Importe',
        'report.liters': 'Litros',
        'report.quantity': 'Cant.',
        'report.bank': 'Banco',
        'report.status': 'Estado',
        'report.date': 'Fecha',
        'report.noPayments': 'Sin pagos',
        'report.monobankInvoices': 'Facturas Monobank: {0}',
        'report.redemptions': 'Canjes',
        'report.redemptionId': 'ID',
        'report.fuelName': 'Combustible',
        'report.redeemedAt': 'Canjeado el',
        'report.noRedemptions': 'Sin cupones canjeados',
        'report.fulfilledOrders': 'pedidos cumplidos',
        'report.profitSummary': 'Resumen de ganancias',
        'report.period': 'Período',
        'report.orders_short': 'Pedidos',
        'report.profit': 'Ganancia',
        'report.avgOrder': 'Prom. Pedido',

        // Forms
        'forms.selectStation': 'Seleccionar Estación',
        'forms.selectFuel': 'Seleccionar Combustible',
        'forms.stationIdPlaceholder': 'ID de Estación (e.g. okko)',
        'forms.stationNamePlaceholder': 'Nombre de Estación (e.g. OKKO)',
        'forms.logoTextPlaceholder': 'Texto del Logo',
        'forms.fuelIdPlaceholder': 'ID de Combustible (e.g. a95)',
        'forms.fuelNamePlaceholder': 'ej. A-95',
        'forms.basePricePlaceholder': 'Precio Base',
        'forms.discountPricePlaceholder': 'Precio con Descuento',
        'forms.qrUrlPlaceholder': 'URL del Código QR o Imagen',
        'forms.packageIdPlaceholder': 'ID del Paquete (e.g. okko_a95_10)',
        'forms.priceUahPlaceholder': 'Precio (UAH)',
        'forms.originalPriceUahPlaceholder': 'Precio Original (UAH)',

        // Dashboard
        'dashboard.availableQrs': 'QRs Disponibles',
        'dashboard.soldQrs': 'QRs Vendidos',

        // Tables
        'table.id': 'ID',
        'table.name': 'Nombre',
        'table.logo': 'Logo',
        'table.color': 'Color',
        'table.station': 'Estación',
        'table.fuel': 'Combustible',
        'table.liters': 'Litros',
        'table.price': 'Precio',
        'table.number': 'N°',
        'table.description': 'Descripción',
        'table.quantity': 'Cant',
        'table.date': 'Fecha',
        'table.amount': 'Importe, UAH',
        'table.debit': 'Débito',
        'table.credit': 'Crédito',
        'table.uah': 'UAH',
        'table.original': 'Original',
        'table.qrPreview': 'Vista Previa QR',
        'table.basePrice': 'Precio Base',
        'table.discountPrice': 'Precio Descuento',
        'table.phone': 'Teléfono',
        'table.email': 'Correo electrónico',
        'table.birthdate': 'Fecha de nacimiento',
        'table.bonusBalance': 'Saldo de Bonos',
        'table.referralCode': 'Código de Referencia',
        'table.referredBy': 'Referido Por',
        'table.role': 'Rol',
        'table.status': 'Estado',
        'users.active': 'Activo',
        'users.inactive': 'Inactivo',
        'users.deleted': 'Eliminado',
        'users.createdAt': 'Fecha de Creación',
        'users.activate': 'Activar',
        'users.deactivate': 'Desactivar',
        'users.activated': 'Usuario activado',
        'users.deactivated': 'Usuario desactivado',
        'users.delete': 'Eliminar',
        'users.deleteConfirm': '¿Eliminar a este usuario? Su cuenta quedará bloqueada permanentemente.',
        'users.deleteSuccess': 'Usuario eliminado',
        'users.deleteFailed': 'Error al eliminar el usuario',
        'users.toggleFailed': 'Error al cambiar el estado del usuario',
        'users.banned_status': 'Bloqueado',
        'users.ban': 'Bloquear',
        'users.unban': 'Desbloquear',
        'users.banConfirm': '¿Bloquear a este usuario? Todas sus sesiones activas terminan de inmediato y deberá iniciar sesión de nuevo tras el desbloqueo.',
        'users.banned': 'Usuario bloqueado',
        'users.unbanned': 'Usuario desbloqueado',
        'users.banFailed': 'Error al cambiar el estado de bloqueo',
        'users.roleChanged': 'Rol actualizado',
        'users.roleChangeFailed': 'No se pudo cambiar el rol',
        'users.editEmail': 'Editar correo',
        'users.emailPrompt': 'Nuevo correo (déjalo vacío para borrarlo). Se envía un enlace de confirmación a la nueva dirección antes de aplicarse.',
        'users.emailChangeSent': 'Enlace de confirmación enviado a la nueva dirección',
        'users.emailCleared': 'Correo borrado',
        'users.emailChangeFailed': 'No se pudo cambiar el correo',

        // Purchases
        'table.vouchers': 'Vales',
        'purchases.refund': 'Reembolso',
        'purchases.refundTitle': 'Confirmar reembolso',
        'purchases.refundConfirm': '¿Reembolsar este pedido? El valor no cumplido de {0} UAH se devolverá al cliente.',
        'purchases.refundNothingToRefund': 'No queda valor no cumplido en este pedido.',
        'purchases.refundStatus.Processing': 'Reembolso pendiente',
        'purchases.refundStatus.Completed': 'Reembolso confirmado',
        'purchases.refundStatus.Failed': 'Reembolso fallido',
        'purchases.refundSuccess': 'Reembolso iniciado: {0} UAH',
        'purchases.refundFailed': 'Error en el reembolso',

        // Vouchers / Import
        'import.title': 'Importar Cupones',
        'import.description': 'Sube archivos PDF o de imagen para importar cupones al sistema.',
        'import.dragDrop': 'Arrastra imágenes de cupones, PDF o ZIP aquí',
        'import.clickSelect': 'Haz clic para seleccionar',
        'import.uploading': 'Subiendo...',
        'import.processing': 'Procesando...',
        'import.largeFileNote': 'Los archivos grandes pueden tardar unos minutos.',
        'import.filesSelected': 'Archivos seleccionados: {0}:',
        'import.completed': 'Completado',
        'import.failed': 'Fallido',
        'import.totalFiles': 'Archivos Totales',
        'import.processed': 'Procesados',
        'import.successful': 'Exitosos',
        'import.failedCount': 'Fallidos',

        // Voucher Table
        'vouchers.id': 'ID',
        'vouchers.image': 'Imagen',
        'vouchers.volume': 'Volumen',
        'vouchers.fuelType': 'Tipo de Combustible',
        'vouchers.provider': 'Proveedor',
        'vouchers.status': 'Estado',
        'vouchers.importedAt': 'Importado El',
        'vouchers.expires': 'Caduca',
        'vouchers.externalId': 'ID Externo',
        'vouchers.total': 'Total',
        'vouchers.filtered': 'Filtrado',
        'vouchers.selected': 'Seleccionado',
        'vouchers.allFuelTypes': 'Todos los tipos de combustible',
        'vouchers.allStatuses': 'Todos los estados',
        'vouchers.allProviders': 'Todos los proveedores',
        'vouchers.all': 'Todos',
        'vouchers.clearFilters': 'Limpiar filtros',
        'vouchers.delete': 'Eliminar',
        'vouchers.deleteConfirmTitle': 'Confirmar eliminación',
        'vouchers.deleteConfirm': '¿Seguro que desea eliminar {0} vales? Esta acción no se puede deshacer.',
        'vouchers.deleteAllTitle': 'Eliminar todos los vales',
        'vouchers.deleteAllConfirm': '¿Seguro que desea eliminar todos los {0} vales? Esta acción no se puede deshacer.',
        'vouchers.deleteAllAction': 'Eliminar todos',
        'vouchers.cancel': 'Cancelar',

        // Voucher Status
        'status.imported': 'Importado',
        'status.available': 'Disponible',
        'status.assigned': 'Asignado',
        'status.used': 'Usado',
        'status.sold': 'Vendido',
        'status.expired': 'Caducado',
        'status.reserved': 'Reservado',
        'status.verificationFailed': 'Verificación Fallida',
        'status.active': 'Activo',
        'status.delivered': 'Entregado',
        'status.pending': 'Pendiente',
        'status.pending_qr': 'QR Pendiente',
        'status.completed': 'Completado',
        'status.failed': 'Fallido',
        'status.started': 'Iniciado',
        'status.verifiedWithWarnings': 'Verificado con Advertencias',

        // Order Statuses
        'order.status.fulfilled': 'Cumplido',
        'order.status.partiallyFulfilled': 'Parcialmente cumplido',
        'order.status.partiallyRefunded': 'Parcialmente reembolsado',
        'order.status.pending': 'Pendiente',
        'order.status.cancelled': 'Cancelado',
        'order.status.refunded': 'Reembolsado',

        // Monobank
        'monobank.status.success': 'Éxito',
        'monobank.status.pending': 'Pendiente',

        // Reconciliation
        'reconciliation.title': 'Auditoría',
        'reconciliation.modalTitle': 'Acta de Conciliación',
        'reconciliation.documentTitle': 'ACTA DE CONCILIACIÓN',
        'reconciliation.periodLabel': 'Período:',
        'reconciliation.periodFrom': 'desde el principio',
        'reconciliation.periodTo': 'hasta hoy',
        'reconciliation.userLabel': 'Usuario:',
        'reconciliation.noOperations': 'Sin operaciones en el período seleccionado',
        'reconciliation.paymentDesc': 'Compra de cupones: {0} {1}L x{2}',
        'reconciliation.redemptionDesc': 'Canje de cupón: {0} {1}L',
        'reconciliation.total': 'Total:',
        'reconciliation.balance': 'Saldo Final:',
        'reconciliation.summaryInfo': 'Información Resumida:',
        'reconciliation.totalOrders': 'Pedidos totales: {0}',
        'reconciliation.vouchersPurchased': 'Cupones comprados: {0} pzas. ({1} L)',
        'reconciliation.vouchersUsed': 'Cupones usados: {0} pzas. ({1} L)',
        'reconciliation.fuel': 'combustible',
        'reconciliation.adminSignature': 'Firma del administrador:',
        'reconciliation.userSignature': 'Firma del usuario:',
        'reconciliation.needAttention': 'Necesita Atención',
        'reconciliation.unfulfilled': 'incumplido',
        'reconciliation.partial': 'parcial',
        'reconciliation.dataIntegrity': 'Integridad de Datos',
        'reconciliation.orphanVouchers': 'vales huérfanos / eventos no procesados',
        'reconciliation.providersDeficit': 'Proveedores con Déficit',
        'reconciliation.importErrors7d': 'errores de importación (7d)',
        'reconciliation.issuesFound': 'Problemas Encontrados ({0})',
        'reconciliation.critical': 'CRÍTICO',
        'reconciliation.warning': 'ADVERTENCIA',
        'reconciliation.threeWayMatch': 'Conciliación Triple · Pedido — Pago — Cumplimiento',
        'reconciliation.threeOrder': 'Pedido',
        'reconciliation.threePayment': 'Pago',
        'reconciliation.threeFulfillment': 'Cumplimiento',
        'reconciliation.threeVouchExp': 'Vales Esp',
        'reconciliation.threeVouchDel': 'Vales Ent',
        'reconciliation.threeMatch': 'Conciliación',
        'reconciliation.threeAge': 'Edad',
        'reconciliation.matchOk': 'OK',
        'reconciliation.matchPartial': 'PARCIAL',
        'reconciliation.matchUnfulfilled': 'INCUMPLIDO',
        'reconciliation.matchCancelled': 'CANCELADO',
        'reconciliation.matchRefunded': 'REEMBOLSADO',
        'reconciliation.matchPartial_refunded': 'PARCIAL + REEMBOLSO',
        'reconciliation.threeRefund': 'Reembolso',
        'reconciliation.moneyLedger': 'Libro mayor · recibido — entregado — reembolsado',
        'reconciliation.received': 'Recibido',
        'reconciliation.deliveredValue': 'Valor entregado',
        'reconciliation.refundedLabel': 'Reembolsado',
        'reconciliation.refundedOrders': 'pedidos reembolsados',
        'reconciliation.refundDesc': 'Reembolso de vales no entregados: {0}',
        'reconciliation.noOrderData': 'Sin datos de pedidos',
        'reconciliation.voucherFunnel': 'Embudo de Vales',
        'reconciliation.loadFailed': 'Error al cargar datos de conciliación',

        // Audit Log
        'auditlog.title': 'Registro de Auditoría',
        'auditlog.eventType': 'Evento',
        'auditlog.aggregate': 'Entidad',
        'auditlog.changedBy': 'Cambiado por',
        'auditlog.date': 'Fecha',
        'auditlog.summary': 'Resumen',
        'auditlog.noEvents': 'Aún no se han registrado eventos.',
        'auditlog.total': 'Total',

        // Error Logs
        'errorlogs.title': 'Registro de Errores',
        'errorlogs.date': 'Fecha',
        'errorlogs.level': 'Nivel',
        'errorlogs.source': 'Origen',
        'errorlogs.message': 'Mensaje',
        'errorlogs.request': 'Solicitud',
        'errorlogs.user': 'Usuario',
        'errorlogs.exception': 'Excepción',
        'errorlogs.exceptionType': 'Tipo',
        'errorlogs.noException': 'Sin detalles de excepción.',
        'errorlogs.noLogs': 'No se han registrado errores.',
        'errorlogs.total': 'Total',
        'errorlogs.clear': 'Limpiar',
        'errorlogs.clearConfirm': '¿Eliminar todos los registros de errores? Esta acción no se puede deshacer.',
        'auditlog.event.ProviderCreated': 'Proveedor creado',
        'auditlog.event.ProviderUpdated': 'Proveedor actualizado',
        'auditlog.event.ProviderDeleted': 'Proveedor eliminado',
        'auditlog.event.FuelAdded': 'Combustible añadido',
        'auditlog.event.FuelRemoved': 'Combustible eliminado',
        'auditlog.event.PriceChanged': 'Precio cambiado',
        'auditlog.event.NominalSetChanged': 'Nominales cambiados',
        'auditlog.event.AdminLoggedIn': 'Administrador conectado',
        'auditlog.event.AdminLoginFailed': 'Error de inicio de sesión del administrador',
        'auditlog.event.UserDeleted': 'Usuario eliminado',
    },
};

interface I18nStore {
    language: Language;
    setLanguage: (lang: Language) => void;
    t: (key: string, ...params: string[]) => string;
}

export const useI18n = create<I18nStore>()(
    persist(
        (set, get) => ({
            language: 'uk', // Default to Ukrainian
            setLanguage: (lang) => set({ language: lang }),
            t: (key: string, ...params: string[]) => {
                const lang = get().language;
                let translation = translations[lang]?.[key] || translations['en']?.[key] || key;
                params.forEach((param, i) => {
                    translation = translation.replace(`{${i}}`, param);
                });
                return translation;
            },
        }),
        {
            name: 'admin-lemberg-language', // Unique storage key for admin
        }
    )
);
