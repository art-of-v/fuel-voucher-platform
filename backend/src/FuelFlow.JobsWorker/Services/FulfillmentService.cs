...
public sealed class FulfillmentService : IFulfillmentService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (vouchersNeeded == 0)
        {
            // Try to mark as fulfilled atomically - only if not already fulfilled
        ...
        for (int i = 0; i < vouchersNeeded; i++)
        {
            ...
            // Try to assign voucher atomically - only if still Available
        ...
        {
            // Mark order as fulfilled atomically
            var updatedCount = await TryMarkOrderFulfilledAsync(order.Id, cancellationToken);
            ...
            if (updatedCount > 0)
            {
                ...
                // Check if OrderFulfilled event already exists (from another instance)
    ...
    /// <summary>
    /// Atomically marks an order as fulfilled (only if currently pending or partially fulfilled).
    /// Returns number of rows updated (0 or 1).
    /// Multiple instances cannot race on this - only one will succeed.
    /// </summary>
