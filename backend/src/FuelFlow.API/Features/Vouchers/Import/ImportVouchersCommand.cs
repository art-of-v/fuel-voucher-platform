...
public sealed class ImportVouchersCommandHandler
{
    ...
    public async Task<ImportVouchersResponse> HandleAsync(ImportVouchersCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await foreach (var page in _pdfRenderer.RenderPagesAsync(request.PdfStream, cancellationToken))
            {
                using (page.Image)
                {
            ...
            import.PageCount = pagesProcessed;
            _logger.LogInformation("Page Count: {PageCount}", pagesProcessed);
            await _context.SaveChangesAsync(cancellationToken);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to render or process PDF pages");
            import.PageCount = pagesProcessed;
