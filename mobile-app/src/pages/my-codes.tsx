                    ...
                    <DialogContent className="sm:max-w-md p-0 bg-transparent border-none shadow-none w-[98vw] sm:w-full h-[98dvh] sm:h-auto flex flex-col overflow-hidden pb-safe">
                      <div className="relative w-full h-full border-2 border-primary bg-black shadow-[0_0_60px_rgba(0,255,128,0.3)] flex flex-col min-h-0">
                        ...
                        <div className="p-4 pb-2 relative overflow-hidden shrink-0 text-center">
                          ...
                          <DialogPrimitive.Close className="absolute top-3 right-3 p-1.5 bg-black/60 hover:bg-primary hover:text-black transition-colors z-30 text-white border-2 border-white/20 rounded-full">
                            <X className="w-4 h-4" />
                          </DialogPrimitive.Close>

                          <div className="relative z-10 pt-2">
                            <h2 className="text-3xl sm:text-5xl font-black text-white font-heading uppercase tracking-tighter text-glow mb-2">{providerName}</h2>

                            <div className="inline-flex items-center justify-center gap-4 sm:gap-6 bg-white/5 border border-white/10 px-4 py-2 rounded-xl backdrop-blur-sm">
                              <div className="text-center">
                                <div className="text-[10px] text-gray-500 uppercase tracking-widest font-mono">Fuel</div>
                                <div className="text-lg sm:text-2xl font-bold text-primary font-heading uppercase leading-tight">{voucher.fuelType}</div>
                              </div>
                              <div className="w-[1px] h-6 bg-white/10" />
                              <div className="text-center">
                                <div className="text-[10px] text-gray-500 uppercase tracking-widest font-mono">Volume</div>
                                <div className="text-lg sm:text-2xl font-bold text-white font-heading uppercase leading-tight">{voucher.amount} L</div>
                              </div>
                            </div>
                          </div>
                        </div>
                          ...
                          <div className={`relative transition-all duration-500 w-full h-full flex flex-col items-center justify-center ${isUsed ? 'opacity-30 grayscale blur-sm' : 'opacity-100'}`}>
                            ...
                            {isUsed && (
                              <div className="absolute inset-0 z-20 flex items-center justify-center">
                                <RotateCcw className="w-10 h-10 text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.8)]" />
                              </div>
                            )}

                            {!isUsed && <div className="absolute w-[280px] h-0.5 bg-red-500 shadow-[0_0_15px_red] z-20 animate-[scan_2s_ease-in-out_infinite] pointer-events-none" />}
                          </div>
                        </div>
                        ...
                        <div className="p-4 flex flex-col items-center gap-3 shrink-0">
                          <button
                            ...
                            className={`w-full max-w-xs py-3.5 flex items-center justify-center gap-3 font-black text-base uppercase tracking-wider transition-all border-2 ${isUsed
                              ? 'bg-transparent text-gray-500 border-gray-700 hover:border-white hover:text-white'
                              : 'bg-primary text-black border-primary hover:bg-primary/90 hover:shadow-[0_0_20px_rgba(0,255,128,0.3)]'
                              }`}
                          >
                            {isUsed ? (
                              <><RotateCcw className="w-4 h-4" />Restore Code</>
                            ) : (
                              <><CheckCircle2 className="w-4 h-4" />Mark as Used</>
                            )}
                          </button>

                          <div
                            onClick={() => copyToClipboard(voucher.externalId || voucher.id)}
                            className="inline-flex items-center gap-2 text-[10px] text-gray-500 hover:text-primary cursor-pointer font-mono tracking-widest uppercase transition-colors py-1"
                          >
